    ...
    <div className="flex-1 bg-[#fdfcfb] py-12 px-6 flex flex-col items-center animate-in fade-in duration-500">
      ...
      <div className="w-full max-w-[640px] space-y-4 mb-10">
        {packages.map((pkg) => (
          <div key={pkg.id} className="relative group">
            {/* Top Badge */}
            {pkg.badge && (
              <div className={`absolute -top-3 right-4 z-10 ${pkg.badgeColor} text-white text-[10px] font-black px-3 py-1 rounded-md shadow-sm`}>
                {pkg.badge}
              </div>
            )}
            ...
            >
              ...
              <div className="flex items-center gap-5">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                  selected === pkg.id ? 'border-[#7c602e] bg-[#7c602e]' : 'border-gray-300'
                }`}>
                  {selected === pkg.id && <div className="w-2 h-2 rounded-full bg-white" />}
                </div>
                <div className="text-left">
                  <p className="text-lg font-bold text-gray-900 leading-tight">{pkg.name}</p>
                  <p className="text-sm text-gray-400 font-medium">{pkg.credits}</p>
                </div>
              </div>
              ...
              <div className="hidden sm:block">
                <img 
                  src="https://cdn-icons-png.flaticon.com/512/2413/2413151.png" // Replace with your actual local coin asset
                  alt="coins" 
              ...
              <div className="text-right">
                <p className="text-xl font-bold text-gray-900 leading-none">{pkg.price}</p>
                <p className="text-[11px] text-gray-400 font-medium uppercase tracking-tight">{pkg.subtitle}</p>
              </div>
            </label>
          </div>
        ))}
      ...
      <div className="w-full max-w-[480px] text-center">
        ...
        <div className="flex items-center justify-center gap-6 opacity-80 grayscale hover:grayscale-0 transition-all">
            ...
            <span className="text-[11px] font-black tracking-tighter text-[#1a1c33] italic">MAESTRO</span>
